<?php
    ...
    $result=mysqli_query($connect,"SELECT * FROM players WHERE playername LIKE '%" .$playername. "%'");
    $export=mysqli_fetch_array($result);
    $arr=explode(",",str_replace("]","",str_replace("[","",$export[1])));
    $foundplayer=$export[0];
